...
Автоответчик использует `openai/gpt-4o-mini` через OpenAI-совместимый endpoint VseGPT. Новое входящее сообщение создаёт одно отложенное состояние `ChatAIState`; следующие сообщения того же клиента заменяют его, поэтому серия реплик не порождает несколько ответов. Перед вызовом модели и перед созданием `OutboundDelivery` повторно проверяется последнее сообщение. Если оно исходящее, задача завершается без ответа.

Контекст строится только из ограниченного числа последних входящих и исходящих сообщений конкретного личного чата. Группы, каналы и боты исключены. Нетекстовые сообщения и неизвестные вопросы получают фиксированную отписку и отметку `needs_human_attention`.

Присутствие администратора обновляется одной короткой записью раз в 20 секунд. Frontend считает администратора активным только при видимой вкладке, сфокусированном окне и недавнем действии мышью/клавиатурой; фоновая вкладка продолжает heartbeat, но не блокирует ИИ. Сервер отдельно хранит время heartbeat и последней реальной активности. При активном администраторе ИИ выключен глобально, если оператор не включил временный online-режим. Ручное исходящее сообщение из CRM или приложения мессенджера отменяет ожидающую задачу и ставит чат на паузу.

В очередь ИИ принимаются только сообщения, чья дата провайдера укладывается в `max_incoming_age_minutes` (по умолчанию 10 минут). Актуальность повторно проверяется непосредственно перед ответом, поэтому сохранённая или восстановленная старая задача не отправит запоздалое сообщение. Модель также может выбрать действие `no_reply` для чистой благодарности, подтверждения или прощания без нового вопроса.

VseGPT вызывается только Celery worker. Redis ограничивает частоту запросов, ошибки повторяются ограниченное число раз, а окончательный сбой переводит обращение человеку. Дополнительный worker или цикл опроса чатов не создаётся.

## Google Контакты

CRM запрашивает только scope `contacts.readonly`. Refresh token хранится в БД и не возвращается frontend. Первая синхронизация загружает контакты страницами, следующие используют `syncToken` и получают только изменения. После этого все сопоставления выполняются локально по нормализованному номеру телефона.

CRM хранит одно общее подключение Google для всех операторов. Новый чат проверяется по локальному индексу без обращения к Google. WhatsApp обычно предоставляет номер в `@c.us`, MAX — в `senderPhoneNumber`, Telegram — только когда телефон доступен подключённому аккаунту. Исходные имена мессенджеров не перезаписываются.
