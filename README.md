...
Реакции, полученные от Telegram, WhatsApp и MAX, отображаются под сообщениями. Отправка реакции из CRM доступна для личных аккаунтов Telegram и напрямую подключённых Telegram-ботов. GREEN-API передаёт события реакций WhatsApp/MAX, но пока не предоставляет отдельный метод их отправки, поэтому в этих каналах кнопка добавления реакции скрыта.

## Дополнительные возможности

- ИИ-автоответчик через VseGPT (`openai/gpt-4o-mini`) отвечает только в личных диалогах, использует отдельный ограниченный контекст каждого чата и передаёт неизвестные вопросы администратору. По умолчанию функция выключена.
- Реальная активность администратора в CRM автоматически приостанавливает ИИ; открытая фоновая вкладка присутствием не считается. Ручной ответ ставит автоответчик на паузу в конкретном диалоге, старые входящие игнорируются, а последнее исходящее сообщение исключает повторный ответ.
- Односторонняя синхронизация Google Контактов подставляет имя из адресной книги, сохраняя под ним исходное имя из мессенджера. Сопоставление новых чатов выполняется локально по номеру телефона.
...
Ручная загрузка диалогов использует журналы входящих и исходящих сообщений GREEN-API за выбранный период. Поэтому она выполняется несколькими запросами, а не последовательно опрашивает сотни записей из `getChats`; в результате отдельно показываются найденные и действительно новые диалоги.
