...
- Поддерживает два типа: `PERSONAL` (Telethon) и `BOT` (pyTelegramBotAPI)
- Сохраняет session strings для Telethon клиентов
...
Singleton менеджер для управления множественными Telethon клиентами.

**Основные функции**:
- `start_client(account)` - Запуск Telethon клиента для аккаунта
- `stop_client(account_id)` - Остановка клиента
- `restart_client(account_id)` - Перезапуск клиента
- `send_message(account_id, chat_id, text)` - Отправка сообщения через Telethon
...
- `sessions_volume` - Сессии Telethon
- `logs_volume` - Логи
...
1. Telethon клиент получает сообщение
