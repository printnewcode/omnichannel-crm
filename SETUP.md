...
Этот файл содержит только практические действия. Техническое устройство проекта, описание контейнеров, переменных, хранилищ и расширенная диагностика находятся в [TECHNICAL_OPERATIONS.md](TECHNICAL_OPERATIONS.md). Короткая инструкция для Beget VPS находится в [deploy/README.md](deploy/README.md).
...
Если нужны ИИ-ответы, создайте API-ключ на [VseGPT](https://vsegpt.ru/) и добавьте:

```dotenv
VSEGPT_API_KEY=ваш_ключ
```

Для Google Контактов включите **People API** в [Google Cloud Console](https://console.cloud.google.com/apis/library/people.googleapis.com), создайте OAuth-клиент типа **Web application** и добавьте в него адрес возврата `https://ВАШ-ДОМЕН/api/google-contacts/callback/`. Затем заполните:
...
### ИИ и Google Контакты

1. Откройте CRM и нажмите кнопку настроек ИИ над списком чатов.
2. Заполните базовый промпт и подтверждённую информацию о компании, затем включите автоответчик.
3. Нажмите кнопку с иконкой контактов рядом с «Загрузить чаты» и разрешите CRM читать Google Контакты.
4. Нажмите эту кнопку ещё раз, чтобы запустить синхронизацию.

Подключается одна общая адресная книга Google для всей CRM. Подключать и запускать её синхронизацию может администратор; остальные операторы сразу видят найденные имена.

ИИ отвечает только в видимых личных чатах. Ручной ответ администратора приостанавливает его в этом чате на один час. Когда администратор активно работает в CRM, ИИ молчит, пока не нажата временная кнопка «Включить ИИ-автоответчик». Вкладка в фоне или окно без действий не считаются присутствием. В настройках можно изменить время бездействия и срок, после которого входящее сообщение уже считается старым.
